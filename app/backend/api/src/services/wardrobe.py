...
def get_wardrobes_by_user_id(db: Database, user_id: str) -> List[models.Wardrobe]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_424_FAILED_DEPENDENCY,
            detail=f"Error retrieving wardrobes for user with id {user_id}: {str(e)}"
        )
...
def get_wardrobes_by_user_role(db: Database, user_id: str, role: str) -> List[models.Wardrobe]:
    """Get wardrobes by user role"""
    try:
        wardrobes_data = wardrobes_repo.find_wardrobes_by_user_role(db, user_id, role)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_424_FAILED_DEPENDENCY,
            detail=f"Error retrieving wardrobes for user with id {user_id} and role {role}: {str(e)}"
        )
...
def create_wardrobe(db: Database, user_id: str,  request: dto_in.CreateWardrobe) -> models.Wardrobe:
...
def update_wardrobe(
    ...
    if existing_wardrobe.user_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the owner can update the wardrobe"
        )
    try:
        result = wardrobes_repo.update_wardrobe_by_id(db, wardrobe_id, update_data)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_424_FAILED_DEPENDENCY,
...
def delete_wardrobe(db: Database, wardrobe_id: str, user_id: str) -> None:
    ...
    try:
        result = wardrobes_repo.delete_wardrobe_by_id(db, wardrobe_id)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_424_FAILED_DEPENDENCY,
